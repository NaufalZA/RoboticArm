...
class ServoControllerApp:
    def __init__(self, root):
        ...
        self.entry_vars = []
        self.servo_buttons = []
        servo_numbers = [1, 2, 3]  # Change to servos 1, 2, 3
        for i, servo_num in enumerate(servo_numbers):
            ...
            slider = ttk.Scale(control_frame, from_=0, to=90, orient="horizontal", length=120, variable=slider_var,
                               command=lambda value, s=servo_num: self.send_command(s, self.round_to_ten(float(value))))
            slider.set(90) # Atur posisi awal ke 90 derajat
        ...
        ttk.Label(instruction_frame, text="Gunakan tombol S1,S2,S3 untuk pilih servo, Arrow keys untuk kontrol", 
                 font=('Arial', 8)).pack()
    ...
    def update_display(self, label, slider_var, entry_var):
        ...
        rounded_value = self.round_to_ten(slider_var.get())
    ...
    def set_servo_from_entry(self, servo_num, entry_var):
        ...
        try:
            ...
            if 0 <= value <= 90:
                rounded_value = self.round_to_ten(value)
                entry_var.set(str(int(rounded_value)))
                ...
                messagebox.showwarning("Invalid Input", "Nilai harus antara 0-90 derajat")
                entry_var.set("90")
        except ValueError:
    ...
    def on_key_press(self, event):
        ...
        if event.keysym in ['Up', 'Right']:
            ...
            new_pos = min(90, self.round_to_ten(current_pos + 5))
            self.sliders[self.selected_servo].set(new_pos)
            
        elif event.keysym in ['Down', 'Left']:
            # Decrease servo position
            current_pos = self.sliders[self.selected_servo].get()
            new_pos = max(0, self.round_to_ten(current_pos - 5))
            self.sliders[self.selected_servo].set(new_pos)
            
        elif event.keysym in ['1', '2', '3']:
            # Select servo with number keys (1, 2, 3)
            if event.keysym == '1':
                self.select_servo(0)
            elif event.keysym == '2':
                self.select_servo(1)
